...
import { mockSala, mockRepositories, mockItem } from "@tests/utils/mocks/mockData";
...
describe("SalaController", () => {
  ...
  beforeEach(() => {
    ...
    jest.spyOn(salaService, 'getPaginatedItensSala').mockResolvedValue({ data: [], totalItems: 0, totalPages: 1, currentPage: 1 });
  });
  ...
  const runErrorTest = (
    ...
    it(`deve retornar erro ${expectedStatus} em caso de erro no serviço para ${controllerMethod}`, async () => {
      ...
      jest.spyOn(salaService, serviceMethod as keyof SalaService).mockRejectedValue(mockError);
  ...
  describe("getSala", () => {
    it("deve retornar as salas paginadas", async () => {
      ...
      jest.spyOn(salaService, 'getPaginatedSalas').mockResolvedValue(paginatedSalas);

      await callControllerMethod('getSala')

      expect(salaService.getPaginatedSalas).toHaveBeenCalledWith({ page: 1, limit: 10 });
  ...
  describe("getSalaByID", () => {
    ...
    runErrorTest('getSalaByID', { id: "1" }, new ServerError("Erro no serviço"), 500);
  });

  describe("updateSala", () => {
    it("deve atualizar a sala com sucesso", async () => {
      await callControllerMethod('updateSala', { id: "1" }, { nome: "Sala atualizada" });

      expect(salaService.update).toHaveBeenCalledWith(1, { nome: "Sala atualizada" });
    ...
    runErrorTest('updateSala', { id: "7" }, new NotFoundError("Sala not found"), 404);
    runErrorTest('updateSala', { id: "1" }, new ServerError("Erro no serviço"), 500);
  });
  ...
  describe("deleteSala", () => {
    ...
    runErrorTest('deleteSala', { id: "4" }, new NotFoundError("Sala not found"), 404);
    runErrorTest('deleteSala', { id: "1" }, new ServerError("Erro no serviço"), 500);
  });
  ...
  describe("getItensSala", () => {
    it("deve retornar os itens da sala", async () => {
      ...
      jest.spyOn(salaService, 'getPaginatedItensSala').mockResolvedValue(mockResponse);

      await salaController.getItensSala(req as Request, res as Response);

      expect(salaService.getPaginatedItensSala).toHaveBeenCalledWith(1, { page: 1, limit: 10 });
      expect(res.status).toHaveBeenCalledWith(200);
    ...
    runErrorTest('getItensSala', { id: "1" }, new NotFoundError("Sala not found"), 404);
    runErrorTest('getItensSala', { id: "1" }, new ServerError("Erro no serviço"), 500);
  });
});
